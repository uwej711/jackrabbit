...
public class WorkspaceResourceImpl extends AbstractResource
            ...
            try {
                ...
                for (Object listEntry : (List<?>) value) {
                    if (listEntry instanceof Element) {
                        ...
                        if (JcrRemotingConstants.XML_CND.equals(e.getLocalName())) {
                            cnd = DomUtil.getText(e);
                        } else if (JcrRemotingConstants.XML_ALLOWUPDATE.equals(e.getLocalName())) {
                            String allow = DomUtil.getTextTrim(e);
                            allowUpdate = Boolean.parseBoolean(allow);
                        }
                    }
                }
                StringReader reader = new StringReader(cnd);
                Session session = getRepositorySession();
                Workspace wsp = session.getWorkspace();
                DefinitionBuilderFactory<NodeTypeTemplate, NamespaceRegistry> factory =
                    new TemplateBuilderFactory(wsp.getNodeTypeManager(), session.getValueFactory(), wsp.getNamespaceRegistry());
                ...
                wsp.getNodeTypeManager().registerNodeTypes(ntts.toArray(new NodeTypeTemplate[ntts.size()]), allowUpdate);
    ...
    protected void initProperties() {
        ...
        try {
            NodeTypeIterator ntti = getRepositorySession().getWorkspace().getNodeTypeManager().getAllNodeTypes();
            Collection<QNodeTypeDefinition> defs = new ArrayList<QNodeTypeDefinition>();
            while (ntti.hasNext()) {
                defs.add(new QNodeTypeDefinitionImpl(ntti.nextNodeType(), new DefaultNamePathResolver(getRepositorySession()), QValueFactoryImpl.getInstance()));
            }
            StringWriter sw = new StringWriter();
            CompactNodeTypeDefWriter writer = new CompactNodeTypeDefWriter(sw, getRepositorySession(), true);
            writer.write(defs);
            writer.close();
            properties.add(new DefaultDavProperty<String>(ItemResourceConstants.JCR_NODETYPES_CND, sw.toString(), false));
        } catch (RepositoryException e) {
